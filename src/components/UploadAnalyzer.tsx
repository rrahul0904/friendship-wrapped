...
export function UploadAnalyzer() {
  ...
  const [lore, setLore] = useState<ThreadTalesLocalLore | null>(null);
  ...
  const analyzeText = useCallback(async (text: string, name = "threadtales-demo.txt", type = "text/plain") => {
    ...
    try {
      const parsed = await analyzeThreadTaleInput({ name, type, size: new Blob([text]).size, text }, dateOrder);
    ...
    } catch (cause) { setStats(null); setLore(null); setError(cause instanceof Error ? cause.message : "Something went wrong while reading that export."); }
  ...
  async function handleFile(file?: File) {
    if (!file || busy) return;
    setStats(null); setLore(null); setError(""); setWarning("");
    ...
    try {
      ...
      await analyzeText(text, file.name, file.type || (file.name.toLowerCase().endsWith(".json") ? "application/json" : "text/plain"));
    } catch { setStats(null); setLore(null); setError("I couldn't read that file. Please export a supported single chat and try again."); }
    finally { setBusy(false); if (fileRef.current) fileRef.current.value = ""; }
  }

  function resetAnalysis() { setStats(null); setLore(null); setError(""); setWarning(""); setDragging(false); if (fileRef.current) fileRef.current.value = ""; fileRef.current?.focus(); }

  return <>
    <div className="uploader" aria-busy={busy}>
      <div className={`drop ${dragging ? "active" : ""}`} onDragOver={(event) => { event.preventDefault(); if (!busy) setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); void handleFile(event.dataTransfer.files[0]); }}>
        <div className="drop-icon" aria-hidden="true">💬</div><h2>Drop your chat export here</h2><p>WhatsApp .txt and single-chat Telegram .json exports are processed locally. Parsing and analytics run in a browser Web Worker when available—the raw file is not uploaded to ThreadTales.</p>
        ...
        <button className="btn btn-primary" onClick={() => fileRef.current?.click()} disabled={busy}>{busy ? "Analyzing in the background…" : "Choose chat export"}</button>
      </div>
      <div className="controls"><label>Story: <select className="select" value={storyMode} onChange={(event) => setStoryMode(event.target.value as StoryMode)} disabled={busy}>{Object.values(STORY_MODES).map((mode) => <option key={mode.id} value={mode.id}>{mode.label}</option>)}</select></label><label>WhatsApp dates: <select className="select" value={dateOrder} onChange={(event) => setDateOrder(event.target.value as DateOrder)} disabled={busy}><option value="auto">Auto / US-first</option><option value="mdy">MM/DD/YYYY</option><option value="dmy">DD/MM/YYYY</option></select></label><button className="btn btn-soft" onClick={() => void analyzeText(makeSampleChat())} disabled={busy}>Use demo chat</button></div>
      {warning ? <div className="notice" role="status">{warning}</div> : null}{error ? <div className="error" role="alert" aria-live="polite">{error}</div> : null}
    </div>
