...
describe("Stripe boundary", () => {
  ...
  it("sends purchase metadata only and uses verified checkout URLs", async () => {
    ...
    const fetchSpy = vi.fn(async () => new Response(JSON.stringify({ id: "cs_test_abc", url: "https://checkout.stripe.test/session" }), { status: 200, headers: { "Content-Type": "application/json" } }));
    vi.stubGlobal("fetch", fetchSpy);
...
describe("AI enrichment privacy", () => {
  ...
  it("uses Responses API with store=false and never sends unprovided identities", async () => {
    const fetchSpy = vi.fn(async () => new Response(JSON.stringify({ model: "test-model", output: [{ content: [{ type: "output_text", text: "Enriched copy" }] }] }), { status: 200, headers: { "Content-Type": "application/json" } }));
    vi.stubGlobal("fetch", fetchSpy);
