...
function participantName(index: number) { return `Person ${index + 1}`; }

export function makeSyntheticChat({ messages, participants = 4, seed = 42, start = new Date(2024, 0, 1, 8, 0, 0), minutesBetweenMessages = 7 }: SyntheticChatOptions) {
  ...
  let state = seed >>> 0;
  const random = () => { state = (state * 1664525 + 1013904223) >>> 0; return state / 0x1_0000_0000; };
  const lines: string[] = [];
